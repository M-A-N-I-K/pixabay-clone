    ...
    return (
        <>
            <div className='flex flex-col items-center'>
                {
                    searchQuery === "" ?
                        <h1 className="mt-6 mb-16 text-3xl sm:text-5xl font-bold tracking-tight text-center md:text-6xl xl:text-7xl">
                            Discover over 2,00,000 <br /><span>free stock images</span>
                        </h1>
                        : ""
                }
                <div className='relative'>
                    <svg className='px-4 w-14 h-8 mt-1 sm:mt-0 sm:w-14 sm:h-12 border-r-[1.5px] border-white absolute left-[5px] z-10 top-[8px] sm:top-[5px]' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 23" fill="none">
                    ...
                    <input type='text' onChange={(e) => { e.preventDefault(), setSearchQuery(e.target.value) }} placeholder='Search' className='inline-flex h-[8vh] sm:h-[10vh] w-[70vw] placeholder-current rounded-xl pl-16 bg-[rgba(217,217,217,0.2)] border-[8.906px] border-transparent backdrop-blur-[25.034873962402344px] justify-between items-center' />
                    <button className='absolute right-4 top-[11px] sm:top-4 px-3 sm:px-4 inline-flex py-1 font-bold border-white rounded-xl border-2 bg-transparent mr-4'>
                        Go!
                    </button>
                </div>
                {
